...
/**
 * @brief Custom exception class for functionality not yet implemented.
 */
class NotImplementedException : public std::logic_error {
public:
  /**
   * @brief Constructs a NotImplementedException with an optional error message.
 ...
 *
 * This macro defines a template specialization that enables SFINAE (Substitution Failure Is Not An Error) to check if a type T is derived
 * from the specified type Type. It relies on the IsDerivedFrom type trait to determine if T is derived from Type.
 *
 * Example Usage:
 * \code{.cpp}
 ...
 * @note This macro should be used before defining a template function or class where the template parameter T needs to be checked if it's
 * derived from a specific base type.
 ...
 * @note This macro should be used before defining a template function or class where the template parameter T needs to be checked if it's
 * derived from a specific base type with additional template arguments.
 * @note This macro uses std::enable_if to perform the type checking at compile-time.
 */
 ...
 *
 * This macro is an extension of IS_CHILD_OF_T macro and allows specifying default template arguments for the derived type T.
 *
 * Example Usage:
 * \code{.cpp}
 * IS_CHILD_OF_TYPE_T_DEFAULTS(BaseType)
 * \endcode
 *
 * @param Type The base type to check if T is derived from.
 * @note This macro should be used before defining a template function or class where the template parameter T needs to be checked if it's
 * derived from a specific base type with default template arguments.
 * @note This macro uses std::enable_if to perform the type checking at compile-time.
 */
#define IS_CHILD_OF_TYPE_T_DEFAULTS(Type) \
  template <typename T = Type, typename = typename std::enable_if<Akr::IsDerivedFrom<T, Type>::value>::type>
